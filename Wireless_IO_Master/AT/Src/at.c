 ...
 */
At_InfoList Wait_Recv(Shell *const shell,
                      const ReceiveBufferHandle pB,
                      const char *resp,
                      uint16_t timeout)
{
#define AT_BASE_TIMEOUT 50
    At_InfoList ret = CONF_TOMEOUT;

    timeout = timeout < AT_BASE_TIMEOUT
                  ? AT_BASE_TIMEOUT
                  : timeout / AT_BASE_TIMEOUT;
    while (!pB->count && timeout--)
    {
        osDelay(AT_BASE_TIMEOUT);
    }
    ...
    if (pB->count)
    {
        ...
        ret = strstr((const char *)&(pB->buf), resp)
                  ? CONF_SUCCESS
                  : (strstr((const char *)&(pB->buf), AT_CMD_ERROR)
                         ? CONF_ERROR
                         : CONF_TOMEOUT);
        /*特殊指令解析回调*/
        At_ParserCallback((char *)pB->buf);
        // #if defined(USING_DEBUG)
        shellPrint(shell, ">[L102->MCU]:%s\r\n", pB->buf);
        // #endif
    }
    ...
    while (1)
    {
        if (shell->read(pData, 0x01))
        {
            ...
            case ENTER_CODE:
            {
                pBuf[len] = '\0';
                shellPrint(shell, "\r\nInput:%s\r\n", pBuf);
                mdClearReceiveBuffer(pH->receiveBuffer); // 清空上一次缓存“LoRa Start!”
