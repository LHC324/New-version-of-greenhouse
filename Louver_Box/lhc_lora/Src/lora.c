...
static void Create_LoraObject(pLoraHandle *pl, pLoraHandle ps)
{
    ...
    if (*pl && ps)
    {
        ...
        // (*pl)->pMap = ps->pMap;
        memcpy(&(*pl)->Check, &ps->Check, sizeof(ps->Check));
...
ListItem_t *Get_OneDevice(pLoraHandle pl, List_t *plist)
{
    ...
    if (pl && plist)
    {
        ...
        LORA_DEBUG("list\t\titems\tdata\r\n%p\t%d\t%d\r\n",
                   plist, plist->uxNumberOfItems, plist->pxIndex->xItemValue);
#endif
...
void Lora_CallBack(pLoraHandle pl, void *pdata)
{
    ...
    if (pl && pdata)
    {
        ...
        /*读取对应寄存器*/
        if (!pd->Mod_Operatex(pd, InputCoil, Read, (pl->Schedule.Event_Id * DIGITAL_INPUT_NUMBERS),
                              rbits, sizeof(rbits)))
        {
#if LORA_USING_DEBUG
