...
#define SLAVE_MAX 0x0F
    ...
    MdbusHandle Modbus;
    Modbus.code = Get_CardId();
    uint8_t slave_id = Modbus.code & 0x0F; // 0x05;

    // Modbus.Slave_Id = SLAVE_ADDRESS;
#if defined(USING_DEBUG)
    Debug("Board id is %#x, slave id is %#x.\r\n", Modbus.code, slave_id);
#endif
...
static void Modbus_Poll(pModbusHandle pd)
{
    ...
    if (pd->Slave.Recive_FinishFlag)
    {
        ...
        if ((Get_ModId(pd) == pd->Slave_Id) &&
            (Get_Data(pd, pd->Slave.RxCount - 2U, MOD_WORD) == ((uint16_t)((crc16 >> 8U) | (crc16 << 8U)))))
        {
#if defined(USING_DEBUG)
            Debug("Data received!\r\n");
            ...
            case ReadInputReg:
            {
                Modus_ReadXRegister(pd);
            }
            break;
            case WriteHoldReg:
            case WriteHoldRegs:
            {
                Modus_WriteHoldRegister(pd);
            }
            break;
#endif
            case ReportSeverId:
...
static void Modus_ReportSeverId(pModbusHandle pd)
{
    ...
    /*读取卡槽与板卡编码*/
    pd->Master.pTbuf[pd->Master.TxCount++] = pd->code; // Get_CardId()
    pd->Mod_Transmit(pd, UsedCrc);
