...
void IRQ_Handle(IRQ_Type irq)
{
    ...
    // p138->pGpio[0].PinState = GPIO_PIN_RESET;
    // p138->pGpio[1].PinState = (irq == DisEnable) ? GPIO_PIN_RESET : GPIO_PIN_SET;

    Write_74HC138(p138);

    // RS485_Rx_Enable();

    // p138->pGpio->pGPIOx->ODR |= p138->pGpio[0].Gpio_Pin;
    // if (irq == DisEnable)
    //     p138->pGpio->pGPIOx->BSRR = p138->pGpio[0].Gpio_Pin;
    // else
    //     p138->pGpio->pGPIOx->BRR = p138->pGpio[1].Gpio_Pin;

    // p138->pGpio->pGPIOx->ODR |= p138->pGpio[0].Gpio_Pin;
    // p138->pGpio->pGPIOx->ODR |= p138->pGpio[1].Gpio_Pin;
}
...
void RS485_Tx_Enable(void)
{
    ...
    p138->pGpio[0].PinState = GPIO_PIN_RESET;
    p138->pGpio[1].PinState = GPIO_PIN_RESET;
...
void RS485_Rx_Enable(void)
{
    ...
    p138->pGpio[0].PinState = GPIO_PIN_SET;
    p138->pGpio[1].PinState = GPIO_PIN_SET;
