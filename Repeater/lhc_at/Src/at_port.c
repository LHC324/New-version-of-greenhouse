...
static unsigned int at_get_count(void)
{
	...
	if (pd->Uart.rx.count > pd->Uart.rx.size) // 截断缓冲区，否则上层可能溢出
	{
		at_printf("@error: recv buf overflow.\r\n");
		pd->Uart.rx.count = pd->Uart.rx.size - 1;
	}

	return (pd->Uart.rx.count);
}
...
static void at_reset_recv_buf(void)
{
	...
	if (pd && pd->Uart.rx.pbuf)
	{
		memset(pd->Uart.rx.pbuf, 0, pd->Uart.rx.size);
		pd->Uart.rx.count = 0;
	}
}
...
static const char *at_get_recv_buf(void)
{
	...
	return ((pd && pd->Uart.rx.pbuf)
				? ((const char *)pd->Uart.rx.pbuf)
				: NULL);
}
...
static void at_send_cmd(char *cmd, unsigned short len)
{
	...
	if (len < pd->Uart.tx.size)
	{
		memcpy(pd->Uart.tx.pbuf, cmd, len);
		pd->Uart.tx.count = len;
	}
...
static uint8_t at_get_resp_flag(unsigned short timeout)
{
	...
	while (!pd->Uart.rx.count && timeout--)
	{
