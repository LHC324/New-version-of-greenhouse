...
static mdVOID mdRTUHook(ModbusRTUSlaveHandler handler, mdU16 addr)
{
    ...
    /*用户名和密码处理*/
    else if (addr >= PARAM_END_ADDR) //GET_PARAM_SITE(Save_Param, System_times[1][2], uint16_t)
    {
        uint16_t udata = 0;
        mdSTATUS ret = mdRTU_ReadHoldRegisters(handler, addr, 1U, (mdU16 *)&udata);
        if (ret == mdFALSE)
        {
#if defined(USING_DEBUG)
...
#define NUMBER_USER_MAX 9999U
            ...
            {
                site = 62 + (addr - 94);
                Endian_Swap((uint8_t *)&udata, 0U, sizeof(uint16_t));
                /*确认数据回传到屏幕*/
                pd->Dw_Write(pd, pd->Slave.pMap[site].addr, (uint8_t *)&udata, sizeof(uint16_t));
            }
        }
#if defined(USING_DEBUG)
...
Gpiox_info card_gpio[] = {
    {.pGPIOx = CARD0_GPIO_Port, .Gpio_Pin = CARD0_Pin},
    {.pGPIOx = CARD1_GPIO_Port, .Gpio_Pin = CARD1_Pin},
    {.pGPIOx = CARD2_GPIO_Port, .Gpio_Pin = CARD2_Pin},
    {.pGPIOx = CARD3_GPIO_Port, .Gpio_Pin = CARD3_Pin},
    {.pGPIOx = CARD4_GPIO_Port, .Gpio_Pin = CARD4_Pin},
    {.pGPIOx = CARD5_GPIO_Port, .Gpio_Pin = CARD5_Pin},
    {.pGPIOx = CARD6_GPIO_Port, .Gpio_Pin = CARD6_Pin},
    {.pGPIOx = CARD7_GPIO_Port, .Gpio_Pin = CARD7_Pin},
    {.pGPIOx = CARD8_GPIO_Port, .Gpio_Pin = CARD8_Pin},
    {.pGPIOx = CARD9_GPIO_Port, .Gpio_Pin = CARD9_Pin},
    {.pGPIOx = CARD10_GPIO_Port, .Gpio_Pin = CARD10_Pin},
    {.pGPIOx = CARD11_GPIO_Port, .Gpio_Pin = CARD11_Pin},
    {.pGPIOx = CARD12_GPIO_Port, .Gpio_Pin = CARD12_Pin},
    {.pGPIOx = CARD13_GPIO_Port, .Gpio_Pin = CARD13_Pin},
    {.pGPIOx = CARD14_GPIO_Port, .Gpio_Pin = CARD14_Pin},
    {.pGPIOx = CARD15_GPIO_Port, .Gpio_Pin = CARD15_Pin},
};
 ...
 * @retval None
 */
static void Set_Card_Led(uint8_t id)
{
    Gpiox_info *pg = card_gpio + id;

    if (pg->pGPIOx && pg->Gpio_Pin)
        HAL_GPIO_TogglePin((GPIO_TypeDef *)pg->pGPIOx, pg->Gpio_Pin);
}
...
#define Get_Data(__ptr, __s, __size)                                                         \
    ((__size) < 2U ? (((__ptr)->pRbuf[__s] << 8U) |                                          \
                      ((__ptr)->pRbuf[__s + 1U]))                                            \
                   : (((__ptr)->pRbuf[__s] << 24U) |                                         \
                      ((__ptr)->pRbuf[__s + 1U] << 16U) | ((__ptr)->pRbuf[__s + 2U] << 8U) | \
                      ((__ptr)->pRbuf[__s + 3U])))
 ...
 * @param  handler modbus句柄
 * @note   RS485失效保护：https://www.amobbs.com/thread-5653834-1-1.html
 * @retval None
 */
static mdVOID portRtuMasterHandle(ModbusRTUSlaveHandler handler, uint8_t target_slaveid)
{
    ...
    Set_Card_Led(Get_ModId(p));

    extern osThreadId TransimtHandle;
    /*Clear the flag after the slave responds correctly*/
    if (xTaskNotify(TransimtHandle, Get_ModId(p), eSetValueWithOverwrite) != pdPASS)
    {
#if defined(USING_DEBUG)
    ...
    break;
    case MODBUS_CODE_2: /*读回的输入线圈状态写回主CPU寄存器组*/
    {
        typeof(ptable->pIRQ) p_target = Find_TargetSlave_AdoptId(ptable, Get_ModId(p));
        if (NULL == p_target)
        {
#if defined(USING_DEBUG)
    ...
    break;
    case MODBUS_CODE_3: /*读回的保持寄存器写回主CPU寄存器组*/
    {
    }
    break;
    case MODBUS_CODE_4: /*读回的输入寄存器状态写回主CPU寄存器组*/
    {
        typeof(ptable->pIRQ) p_target = Find_TargetSlave_AdoptId(ptable, Get_ModId(p));
        if (NULL == p_target)
        {
...
/*
    ...
    接口：帧间隙监控
*/
static __inline mdVOID portRtuTimerTick(ModbusRTUSlaveHandler handler, mdU32 ustime)
{
