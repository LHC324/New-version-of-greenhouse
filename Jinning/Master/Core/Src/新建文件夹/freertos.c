...
#if (TEST_BX_SIZE)
    rtc_t cur_rtc;
    uint32_t now_time, start_time = 8U << 8 | 30U, end_time = 18U << 8;
#endif
...
#if (TEST_BX_SIZE)
    Close_Qx(9U), Open_Qx(10U), Close_Qx(11U), Open_Qx(12U); //----@
#endif

#define __USER_CONTRL_PARAMTER_CALCULATION
    {
      ret = mdRTU_ReadInputRegisters(Slave1_Object, INPUT_ANALOG_START_ADDR, (BX_SIZE + TEST_BX_SIZE) * 2U, (mdU16 *)&ps->User);
      if (ret == mdFALSE)
      {
#if defined(USING_DEBUG)
      ...
      for (float *p = ps->User.C02, *pu = (float *)ps->Param.C02_limit, *pinfo = usinfo.C02;
           ...
           ++p, pu += 2, pinfo++)
      {
          *p = Get_Target(*p, *pu, *(pu + 1U)) < 0.0F ? 0 : *p;
          *pinfo = *p;
      }
#endif
...
/*reset timer*/
    ...
    now_time = cur_rtc.time.hours << 8U | cur_rtc.time.minutes;
...
//   // };
//   // osDelay(5000);
//   // /*Proactively requesting local RTC time updates on the Devon screen*/
...
//     else
//     {
//       for (float *puser = &urinfo.Ptank_M; puser < &urinfo.Ptank_M + (BX_SIZE + TEST_BX_SIZE); puser++)
//       {
// #if defined(USING_DEBUG)
...
//       }
//       Dwin_Object->Dw_Write(Dwin_Object, PRESSURE_OUT_ADDR, (uint8_t *)&urinfo, (BX_SIZE + TEST_BX_SIZE) * sizeof(float));
//       osDelay(DELAY_TIMES);
//     }
//     /*Report error code*/
