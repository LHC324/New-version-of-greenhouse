...
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
#define CURRENT_HARDWARE_VERSION 150
#define CURRENT_SOFT_VERSION 191
...
void Report_Backparam(pDwinHandle pd, Save_Param *sp)
{
  ...
  memcpy(pdata, &sp->System_times, sizeof(sp->System_times));
  for (uint16_t *p = (uint16_t *)pdata;
      (actual_size > sizeof(sp->System_times)) &&
       p < (uint16_t *)pdata + sizeof(sp->System_times) / sizeof(sp->System_times[0][0]);
       ++p)
  {
#if defined(USING_DEBUG)
    shellPrint(Shell_Object, "%p\t%u\r\n", p, *p);
#endif
    Endian_Swap((uint8_t *)p, 0U, sizeof(sp->System_times[0][0]));
  }
  pd->Dw_Write(pd, SYS_START_WH_ADDR, (uint8_t *)pdata, sizeof(sp->System_times));
  ...
  for (;;)
  {
    ...
    if (xQueueReceive(CodeQueueHandle, &interrupt_id, osWaitForever) == pdPASS)
    {
      ...
      /*Ensure that the previous round of interrupt has been handled correctly*/
      {
        ...
        if ((ps->Param.Slave_IRQ_Table.IRQ_Table_SetFlag == SAVE_SURE_CODE))
        {
          if (p_current->site > ps->Param.Slave_IRQ_Table.Max_Irq)
            continue;

          for (irq = sp_irq->pIRQ;
               irq < sp_irq->pIRQ + sp_irq->TableCount;
               irq++)
          {
            if ((1U << irq->SlaveId) == p_current->site)
            {
              /*Assign priority to the current interrupt source*/
              p_current->Priority = irq->Priority;
              break;
            }
          }
          /*Filter invalid card slot interrupt*/
          if (irq == sp_irq->pIRQ + sp_irq->TableCount)
            continue;
        }
        else // Solve the problem of inability to filter invalid interrupt sources after the existence of an empty board card
        {
          if (p_current->site > ps->Param.Slave_IRQ_Table.Max_Irq)
            ps->Param.Slave_IRQ_Table.Max_Irq = p_current->site;
        }
 ...
 * @retval true/false
 */
static bool Check_Bytes_State(mdBit *p_current, mdBit *p_last, uint8_t size)
{
  bool ret = false;
  if (p_current && p_last && size)
  {
...
#define BIT_STEP 16U
    Shell *sh = shellGetCurrent();
    shellPrint(sh, "wbits value\r\n");

    for (uint8_t i = 0; i < sizeof(wbit); ++i)
    {
      if (!(i % BIT_STEP))
        shellPrint(sh, "\r\n[%02x - %02x]: ", i , i + (BIT_STEP - 1));
      
      shellPrint(sh, "%x ", wbit[i]);
    }
    shellPrint(sh, "\r\n");
#undef BIT_STEP
}
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0) | SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC),
                 see_wbit, see_wbit, View Control Bit Status.);

uint8_t distingulsh_times_slot(uint32_t cur, uint32_t start, uint32_t end)
{
    if (start == end) // Same starting and ending points: working 24/7
        return 1;

    if (start < end) // Not spanning days
...
#if (TEST_BX_SIZE)
  rtc_t cur_rtc;
  uint32_t now_time , start_time, end_time;
#endif

  Save_User usinfo;
  static bool mutex_flag[] = {false, false, false, false, false, false, false};
  /*Solve the problem of screen background parameter delay*/
  // osDelay(2000);
  // Report_Backparam(Dwin_Object, &ps->Param);
...
#define __USER_CONTRL_PARAMTER_CALCULATION
    {
      ret = mdRTU_ReadInputRegisters(Slave1_Object, INPUT_ANALOG_START_ADDR,
                                     (BX_SIZE + TEST_BX_SIZE) * 2U, (mdU16 *)&ps->User);
      if (ret == mdFALSE)
      {
#if defined(USING_DEBUG)
...
#if (TEST_BX_SIZE)
      for (float *p = ps->User.C02, *pu = (float *)ps->Param.C02_limit, *pinfo = usinfo.C02;
           p < ps->User.C02 + TEST_BX_SIZE;
           ++p, pu += 2, pinfo++)
      {
        float value = Get_Target(*p, *pu, *(pu + 1U));
        *p = value < 0.0F ? 0 : value;
        *pinfo = *p;
      }
#endif
...
#define USER_INPUTREGS_ADDR 256U
      ...
      mdRTU_WriteInputRegisters(Slave1_Object, USER_INPUTREGS_ADDR,
                                (BX_SIZE + TEST_BX_SIZE) * 2U, (mdU16 *)&ps->User);
...
#if defined(USING_DEBUG)
        if (g_Debug_Bits & DEBUG_APP_BITT)
          shellPrint(Shell_Object, "in\tval\r\n%d\t%d\r\n", i, sbit);
#endif
        goto __no_action;
      }
    }
...
/*reset timer*/
    ...
    linux_stamp_to_std_time(&cur_rtc, get_dwin_rtc(), 8);

    now_time = ((uint32_t)cur_rtc.time.hours) << 16U | cur_rtc.time.minutes << 8U | cur_rtc.time.seconds;
    start_time = ps->Param.System_times[0][0] << 16U | ps->Param.System_times[0][1] << 8U | ps->Param.System_times[0][2];
    end_time = ps->Param.System_times[1][0] << 16U | ps->Param.System_times[1][1]<< 8U | ps->Param.System_times[1][2];  
    // Not within the allowed startup time period
    if (!distingulsh_times_slot(now_time, start_time, end_time))
    {
      shellPrint(Shell_Object, "now\t\tstart\tend\r\n%#x\t%#x\t%#x\r\n", now_time, start_time, end_time);
      goto __no_action;
    }
#endif
    ...
    {
#if (TEST_BX_SIZE)
      bool *mux = mutex_flag + 3U;
      uint8_t *pbit = wbit + 10U;
      uint8_t *bits = pbit;

      for (float *p = ps->User.C02; p < ps->User.C02 + TEST_BX_SIZE; ++p)
      {
        if (*p < ps->Param.C02_range_min)
        {
          *mux = true;
        }

        if (*p > ps->Param.C02_range_max)
        {
          *mux = false;
        }
        ...
        *pbit = *mux ? true : false;
        ...
        if (!(rbit & (1 << 0U))) //I0->Q10 、Q11
          bits[0] = bits[1] = false;

        if (!(rbit & (1 << 3U))) //I3->Q12 、Q13
          bits[2] = bits[3] = false;
...
#endif
          (rbit & ((1 << 0U) | (1 << 2U))) ? Open_Qx(5U), Close_Qx(6U) : false;
          (rbit & (1 << 2U)) ? Open_Qx(7U), Close_Qx(8U) : false;
#if defined(USING_DEBUG_APPLICATION)
...
#define B1C1
    ...
    {
#if (TEST_BX_SIZE)
      mutex_flag[3] = mutex_flag[4] = mutex_flag[5] = mutex_flag[6] = false;
#endif
...
#define NE_TIMES 10U
  ...
  // #if defined(USING_DEBUG)
  //     shellPrint(Shell_Object, "Error: Invalid interrupt source:\r\nsite\tid\ttable\r\n%#x\t%d\t%d\r\n",
  //                rp_irq->site, cur_id, sp_irq->TableCount);
  // #endif
  //     return;
  //   }
...
void Report_Task(void const *argument)
{
  ...
  Report_Backparam(Dwin_Object, &Save_Flash.Param);
  osDelay(DELAY_TIMES);
  Dwin_Object->Dw_Read(Dwin_Object, DWIN_SYSTEM_READ_RTC_ADDR, 0x04);
  osDelay(DELAY_TIMES);
  /* Infinite loop */
  for (;;)
  {
    ...
    {
      for (float *puser = &urinfo.Ptank_M; puser < &urinfo.Ptank_M + (TEST_BX_SIZE + BX_SIZE); puser++)
      {
#if defined(USING_DEBUG)
      ...
      Dwin_Object->Dw_Write(Dwin_Object, PRESSURE_OUT_ADDR, (uint8_t *)&urinfo,
                            (BX_SIZE + TEST_BX_SIZE) * sizeof(float));
      osDelay(DELAY_TIMES);
    }
    /*Report error code*/
...
void Modbus_Callback(void const * argument)
{
  ...
  extern void dwin_rtc_value_inc(void);
  dwin_rtc_value_inc();
#if defined(USING_DEBUG)
